...
#'
#' Based on data, this function build the jags model that will be used by runjags.
#' The model is either an escropath or an escroc model depeding on the data
#'
...
return(
  paste(
    ...
    compute_mean_signature,
    observation_process_signatures,
    ifelse(escropath, flow_computation, ""),
    ifelse(escropath, priors_ecopath_model, ""),
    ifelse(escropath, priors_top_pred, ""),
    ifelse(escropath, observations_biomass, ""),
    ifelse(escropath, cons_top, ""),
    ifelse(escropath, cons, ""),
    ifelse(escropath, equilibrium, ""),
    "}",
    sep = "\n"
  )
)
